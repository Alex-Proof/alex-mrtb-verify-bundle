#!/usr/bin/env bash
# Beweist drei unabhaengige Dinge zu PR #38 von alex-controlled-agent-demo,
...
curl -sO https://bewusstki.de/downloads/verify-bundle/demo-pr-38.json
...
const bundle = require('./demo-pr-38.json');
...
function resolve(keyId, activeId, activePem, outFile, label) {
  ...
  console.log('Hinweis: PR #38 nutzt einen abgeloesten ' + label + '-Schluessel (' + match.key_id + '), historischen Wert verwendet.');
}
...
node verifier/dist/verify.js demo-pr-38.json trusted-public-key.pem trusted-approval-key.pem

echo "== [4/4] L2 -- Diff im Paket gegen den echten Diff auf GitHub nachrechnen =="
BASE=$(node -e "console.log(require('./demo-pr-38.json').controller_evidence.repository_state.base_commit)")
RESULT=$(node -e "console.log(require('./demo-pr-38.json').controller_evidence.repository_state.result_commit)")
...
echo "PR ansehen: https://github.com/Alex-Proof/alex-controlled-agent-demo/pull/38"
